...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _loadInitialData() async {
    ...
    await Future.wait([_loadDepartments(), _loadRoles()]);
  ...
  Future<void> _signUp() async {
    ...
    try {
      ...
      final uucmsId = _uucmsController.text.trim();
      ...
      if (userId != null) {
        final profile = {
          ...
          if (_selectedRoleId != null &&
              _getRoleNameById(_selectedRoleId!) == 'student')
            'uucms_id': uucmsId,
        };

        await Supabase.instance.client.from('profiles').insert(profile);
  ...
  String _getRoleNameById(String id) {
    final role = _roles.firstWhere((r) => r['id'] == id, orElse: () => {});
    return role['name'] ?? '';
  }

  Widget backButton(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(top: 0.0, left: 10.0), // Added left margin
